...
public class BreakFloor : MonoBehaviour
{
    ...
    [Tooltip("乗った瞬間の演出時間")] [SerializeField] float whiteEffectTime = 0.05f;
        ...
        for (int j = 0; j < 2; j++)
        {
            ...
            if (flg)
            {
                ...
                if (noticeMaterial != null)
                {
                    ...
                    {
                        if (isRender[i]) { allObj[i].GetComponent<MeshRenderer>().material = originMat; }
                    }
                }
            }
    ...
    IEnumerator WhiteEffect()
    {
        ...
        if (noticeMaterial != null)
        {
            for (int i = 0; i < allObj.Count; i++)
            {
                if (isRender[i]) { allObj[i].GetComponent<MeshRenderer>().material = noticeMaterial; }
            }
        }
        ColorChange(white);
